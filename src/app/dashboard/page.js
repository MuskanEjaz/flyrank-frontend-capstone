      ...
      <div className="grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
        <article className="card-surface section-block">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h3 className="text-xl font-semibold text-[color:var(--foreground)]">Recent tasks</h3>
          ...
          <ul className="mt-6 space-y-3">
            {recentTasks.map((task) => (
              <li key={task.title} className="flex items-center justify-between rounded-2xl border border-[color:var(--border)] bg-slate-50 px-4 py-3">
                <span className="font-medium text-[color:var(--foreground)]">{task.title}</span>
        ...
        <article className="card-surface section-block">
          ...
          <Link href="/tasks/new" className="solid-action mt-6 inline-flex rounded-full bg-[color:var(--primary)] px-4 py-2 text-sm font-semibold hover:opacity-90">
            Add a task
          </Link>
        </article>
      </div>
